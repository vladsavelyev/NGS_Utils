...
(added   <- genes_sum %>% filter(is_key, !symbol %in% prev$symbol))
(removed <- genes_sum %>% filter(symbol %in% prev$symbol, !is_key))

# play around:
genes_sum %>% filter(is_key) %>% filter(tier1)
genes_sum %>% filter(is_key) %>% filter(cosm1 & cancermine & !ngc)  # 0
genes_sum %>% filter(is_key) %>% filter(ngc) %>% arrange(desc(n))
genes_sum %>% filter(is_key & !tumorsuppressor & !oncogene & !fusion & !germ) %>% arrange(desc(n))
...
library(lubridate)
# tsv with metadata:
genes_sum %>% filter(is_key) %>% select(-is_key) %>% write_tsv(str_c("archive/umccr_cancer_genes.", lubridate::today(), ".tsv", sep = ""))
genes_sum %>% filter(is_key) %>% select(-is_key) %>% write_tsv(str_c("umccr_cancer_genes.latest.tsv", sep = ""))
# plain list of genes:
genes_sum %>% filter(is_key) %>% .$symbol %>% write_lines(str_c("archive/umccr_cancer_genes.", lubridate::today(), ".genes", sep = ""))
genes_sum %>% filter(is_key) %>% .$symbol %>% write_lines(str_c("umccr_cancer_genes.latest.genes", sep = ""))
# plain list of only tumor suppressor genes:
genes_sum %>% filter(tumorsuppressor) %>% .$symbol %>% write_lines(str_c("archive/umccr_cancer_genes.tsgenes.", lubridate::today(), ".genes", sep = ""))
