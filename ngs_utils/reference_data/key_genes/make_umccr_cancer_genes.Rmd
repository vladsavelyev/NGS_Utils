...
genes_sum <- genes_hmf %>%
  ...
  mutate(
    tier1 = ngc | cosm1 | cancermine | driver | n >= 4,  # well curated cancer genes
...
genes_sum %>% filter(key) %>% select(-key) %>% write_tsv(tsv_fp)

list_fp = str_c("umccr_cancer_genes.", lubridate::today(), ".genes", sep = "")
genes_sum %>% filter(key) %>% (function(d) {d$symbol}) %>% write_lines(list_fp)

tsgenes_fp = str_c("umccr_cancer_genes.tsgenes.", lubridate::today(), ".genes", sep = "")
genes_sum %>% filter(tumorsuppressor) %>% (function(d) {d$symbol}) %>% write_lines(tsgenes_fp)
```
