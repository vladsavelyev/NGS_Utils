...
predispose   <- read_tsv(str_c(folder, "pcgr_predispose_genes.txt"), col_names = c('symbol'))
...
genes_sum <- genes_tsg %>%
  ...
  summarise(
    ...
    fusion = any(ngc_fu, na.rm = T) | any(cos_fu, na.rm = T),
  ) %>%
  mutate(
    tumorsuppressor = ngc_ts | cm_ts > 0 | cos_ts | tsg_ts,
    oncogene = ngc_og | cm_og > 0 | cos_og | tsg_og
  ) %>% 
  mutate(keep = ngc | cosm1 | cancermine | germ | n >= 2) %>% 
...
```{r write_results}
genes_sum %>% filter(keep) %>% select(-keep) %>% write_tsv("/Users/vsaveliev/git/NGS_Utils/ngs_utils/reference_data/key_genes/umccr_cancer_genes.20190122.tsv")
```
