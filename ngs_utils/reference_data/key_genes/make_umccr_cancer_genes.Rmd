...
genes_sum <- genes_hmf %>%
  ...
  mutate(
    tier1 = ngc | cosm1 | cancermine | cacao | driver | n >= 4,  # well curated cancer genes
...
genes_sum %>% filter(key) %>% .$symbol %>% write_lines(list_fp)

tsgenes_fp = str_c("umccr_cancer_genes.tsgenes.", lubridate::today(), ".genes", sep = "")
genes_sum %>% filter(tumorsuppressor) %>% .$symbol %>% write_lines(tsgenes_fp)
```
