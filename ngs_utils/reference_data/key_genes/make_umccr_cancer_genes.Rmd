...
genes_sum <- genes_hmf %>%
  ...
  mutate(
    tier1 = ngc | cosm1 | cancermine | n >= 4,  # well curated cancer genes
    keep = tier1 | germ | hmf_fu | n >= 2       # + not so confident + predisposition + known fusions
  ) %>% 
  select(-ends_with("_og"), -ends_with("_ts"), -starts_with("ngc_"), -driver, -ngc, -cosm1, -cancermine)

genes_sum %>% filter(symbol == "TP53")
```

```{r compare_to_previous}
prev = read_tsv("umccr_cancer_genes.2019-01-22.tsv")

(added   <- genes_sum %>% filter(keep, !symbol %in% prev$symbol))
(removed <- genes_sum %>% filter(symbol %in% prev$symbol, !keep))

genes_sum %>% filter(keep) %>% filter(tier1)

genes_sum %>% filter(keep) %>% filter(cosm1 & cancermine & !ngc)  # 0

genes_sum %>% filter(keep) %>% filter(ngc) %>% arrange(desc(n))
genes_sum %>% filter(keep & !tumorsuppressor & !oncogene & !fusion & !germ) %>% arrange(desc(n))
genes_sum %>% filter(cos_fu & !hmf_fu)
```

```{r write_results}
library(lubridate)
res_fp = str_c("umccr_cancer_genes.", lubridate::today(), ".tsv", sep = "")
genes_sum %>% filter(keep) %>% select(-keep) %>% write_tsv(res_fp)
```
